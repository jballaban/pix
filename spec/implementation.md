...
**`organize`'s moves re-upload on Synology Drive Client (measured).** The remaining sync question was whether a client recognizes `organize`'s library-wide file **moves** as renames (cheap, server-side move) or as delete + re-upload. Measured empirically against Synology Drive Client on Windows (60 files moved between folders via a template change, read back from the client's `history.sqlite`): **every moved file was re-uploaded** — logged as remove-from-old-path (`action=18`) + upload-to-new-path (`action=24`), 60/60, with the client running live. There is no rename detection. So a template change or full reorganize of a synced vault re-transfers the moved fraction of the library; treat `organize` reshapes as expensive on a synced library and do them rarely. (Day-to-day `migrate` of new files uploads only those files, which is unavoidable regardless.) Pausing the client during a run does **not** avoid this — on resume it still sees the files at new paths and re-uploads; pausing only avoids transient-marker churn. Hard links exist only under `.pix/local/checkout` (excluded), so once `.pix/local` is excluded the client never has to reason about links.
