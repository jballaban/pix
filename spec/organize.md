...
- **Same plan/edit/confirm flow as migrate.** Generate a plan, open in `$EDITOR`, show summary, `Apply? [Y/n]` (defaults to Y; the editor pass is the review step). Conservation captures any displaced state under `.pix/runs/<run-id>/` (see [library.md](library.md#file-layout)).
