...
Files never move between folders during migrate. Only [organize](organize.md) does that. The **one carve-out** (designed, not yet built) is the **import-ingest pre-pass**: it moves `VERIFIED` device imports from `.pix/local/import/` into a flat `<root>/incoming/` before the in-place pass, so they enter the pipeline like any other file — mirroring the `errors/`/`stash/` restore passes but into the tree (import files have no in-tree origin to restore to). See [import.md → Ingestion (migrate pre-pass)](import.md#ingestion-migrate-pre-pass). Cross-file deduplication is a separate operation: see [dedupe.md](dedupe.md).
