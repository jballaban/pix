...
7. **Reap empty `.pix` workdirs.** On the way out (any exit path), remove `.pix\{errors,staging,stash}\` if they've become empty — bottom-up, so an emptied mirrored `errors\` tree goes too. These are the `.pix` subfolders migrate owns: it resolves/restores `errors\`, wipes `staging\`, and writes `stash\`. Best-effort — a non-empty tree, or one the user is currently `cd`'d into (Windows holds the directory handle), just fails `rmdir` and is left in place. `runs\` and `cache\` are never reaped (rollback data; rebuildable cache). Silent to the console; logged to `plan.log`.
...
- **CONVERT failures move the source into `.pix/errors/` and continue.** A CONVERT action that fails on the conversion step itself (Pillow can't decode an HEIC, ffmpeg can't read a corrupted MOV, …) is logged to `apply.log` as `Failed`, then the source file is moved into `<library>/.pix/errors/` with a sidecar at `<...>.errorinfo` recording the `original_path`, the error message, the failure timestamp, the run-id, and the **pix version that produced the failure**. The run continues to the next plan line. At end of run, the console prints just the **count** of quarantined files and the `.pix/errors/` path (not each file — the errors tree, mirrored by source path with per-file `.errorinfo` sidecars, is where the user reviews specifics) and migrate exits non-zero.

  **The errors tree mirrors the source path.** Rather than an opaque flat name, the moved file is placed at a path that *mirrors* its source under the errors tree — the same drive-folding scheme the [cache](#caching) uses (`pix.cache_base.mirror_under` / `unmirror_under`): `G:\photos\2014\foo.mp4` → `.pix/errors/G/photos/2014/foo.mp4` (+ `.errorinfo` alongside). This diverges from [stash](#stash-action)'s opaque naming on purpose. The error message in the sidecar is regeneratable, but the **original path is not** — encoding it in the file's *location* means a lost or corrupt sidecar no longer loses provenance: the source path is recovered by reversing the mirror, and the file restored to exactly where it came from. (Trade-off: deep sources can approach the Windows 260-char path limit under the mirror — the same constraint the cache tree already lives with; errors are rare, so acceptable.)

  **Auto-retry on version bump.** Migrate's cleanup phase restores any `.pix/errors/` entry whose origin is known and whose recorded `pix_version` differs from the running `pix.__version__` (or is absent — legacy sidecars from pre-v0.1.86, and any sidecar-less mirrored entry, are treated as stale). The file goes back to its source path — taken from its mirrored *location*, or for legacy flat entries from the sidecar's `original_path` — where the upcoming plan-gen pass re-proposes the CONVERT under the current code. Files quarantined by the *current* version are left in place — retrying the same code would hit the same failure, so the user must intervene (upgrade pix, restore from backup, or hard-delete the entry). The full restore log lands in `plan.log`, but the cases the user must act on are also echoed to the console: how many files remain quarantined under the current version (and how to retry), how many were restored for re-attempt this run, and any entries that couldn't be restored (occupied target slot, move failed, or a lone sidecar whose data file is gone). Without this, a library whose only outstanding work is a current-version quarantine would print a bare "Nothing to do." with no hint the file exists.

  **Origin-less orphans.** Only a *legacy flat* entry (pre-mirror layout) with no recoverable origin — neither a mirrored location nor a usable sidecar — is a true orphan. Rather than leave it stranded and invisible, the cleanup phase moves it into the folder currently being migrated and lets plan-gen re-attempt it this run; if it fails again, apply re-quarantines it *with* a fresh sidecar in the mirrored layout, so provenance is restored from then on. This is silent on the console — the file simply appears as an ordinary plan line (or, on re-failure, via the quarantine reporting above). It never overwrites a file already present in the target folder. New-layout entries never reach this path: a sidecar-less *mirrored* file is restored to its true source via its location.
...
- **A tag write that doesn't persist triggers a salvage attempt, then quarantine.** ExifTool can exit 0 yet write *nothing* when a file is structurally off (a truncated video `mdat`/unknown trailer; a PNG mislabeled `.jpg`; a JPEG with a proprietary trailer and no EOI marker — all hard errors `-m` doesn't suppress); it reports `0 image files updated`, and `ExifToolSession.write_tags` parses that count and raises `TagWriteFailed`. Apply then tries to **salvage the file into a clean, taggable form** before giving up:
...
The distinction: CONVERT failures and non-persisting tag writes are almost always a property of the *source file* (truncated, corrupted, unreadable/unwritable for that specific input); other failures are almost always a property of the *environment* (ExifTool wedged, disk full, AV holding a lock). Timeouts always halt regardless of the action — see [implementation.md → On timeout: halt for investigation](implementation.md#on-timeout-halt-for-investigation).
