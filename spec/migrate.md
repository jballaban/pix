...
- **A tag write that doesn't persist moves the file into `.pix/errors/` and continues** — same path as a CONVERT failure. ExifTool can exit 0 yet write *nothing* when a file is structurally damaged (a truncated `mdat` / unknown trailer raises a hard error that `-m` doesn't suppress); it reports `0 image files updated`. `ExifToolSession.write_tags` parses that count and raises `TagWriteFailed`, which apply treats as a per-file data problem (the file's bytes are untouched, but it can't hold `pix:*` tags) and quarantines. **Critically, the failed write is NOT recorded into the metadata cache** — otherwise the cache would claim tags the file never received, and the file would masquerade as migrated until a later op (organize's `pix:OriginalPath` check, a re-read on cache miss) tripped over the divergence.
