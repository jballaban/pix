...
- **CONVERT failures move the source into `.pix/errors/` and continue.** A CONVERT action that fails on the conversion step itself (Pillow can't decode an HEIC, ffmpeg can't read a corrupted MOV, …) is logged to `apply.log` as `Failed`, then the source file is moved into `<library>/.pix/errors/` with a sidecar at `<...>.errorinfo` recording the `original_path`, the error message, the failure timestamp, the run-id, and the **pix version that produced the failure**. The run continues to the next plan line. At end of run, the console prints the list of moved paths and migrate exits non-zero.
